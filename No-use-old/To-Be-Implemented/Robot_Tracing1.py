...
def get_hue_mask(frame, lower, upper):
    ...
    # Create a binary mask where the pixels within the hue range are set to 255 (white) and the rest are set to 0 (black)
    hue_mask = cv2.inRange(hue_channel, lower, upper)
...
try:
    while True:
        ...
        if hsv_ranges == {}:
            for color in colors:
                print(f"Select Hue value for {color}")
                hue_value = get_hue_mask(frame)

                # get the min and max Hue values based on the selected Hue value
                lower = max(0, hue_value - 5)
                upper = min(179, hue_value + 5)  # Hue ranges from 0 to 179 in OpenCV

                print(lower)
                print(upper)

                hsv_ranges[color] = {
                    "lower": np.array([lower, 0, 0], dtype=np.uint8),  # Assume saturation and value to be high
                    "upper": np.array([upper, 255, 255], dtype=np.uint8)  # Assume saturation and value to be high
                }
            print(hsv_ranges)
        ...
        if frame_corners is not None:  # This is a new conditional block
            ...
            min_contour_area = 15  # adjust this value to suit your needs
